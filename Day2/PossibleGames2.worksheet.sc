...
class Game(val id: Int, val grabs: List[Grab]) {
    ...
    def power() : Int = {
        grabs.map(g => g.red).max * grabs.map(g => g.green).max * grabs.map(g => g.blue).max 
    }
}
...
var games = lines.map(retrieveGame).toList
// sum de ids of all possible games
games.filter(game => game.isPossible()).map(g => g.id).reduce((x, y) => x + y)
// sum of all powers
games.map(g => g.power()).reduce((x, y) => x + y)
